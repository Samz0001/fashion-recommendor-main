...
const FashionRecommender: React.FC = () => {
  ...
  const handleRecommend = useCallback(async () => {
    ...
    try {
      ...
      // ❗ FINAL CORRECT WAY → Send Gender, Occasion, Season as QUERY PARAMETERS
      const res = await api.post(
        `/get_recommendation/?Gender=${gender}&Ocassion=${occasion}&Season=summer`,
        form,
        {
    ...
    <ToolCard title="Fashion Recommender" icon={<Camera size={24} />}>
      <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            <h3 className="text-lg font-medium text-gray-800 dark:text-white mb-3">Recommendations</h3>

            {isLoading && !recommendationImage ? (
              ...
              >
                <img src={recommendationImage} alt="Recommended Outfit" className="w-full h-auto rounded-lg" />
              </motion.div>
            )}
          </div>
        </div>
